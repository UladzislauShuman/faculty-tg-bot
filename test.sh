...
echo -e "     ${YELLOW}[all, markdown, hybrid, test, force-index]${NC}"
...
if [ "$MENU_CHOICE" == "1" ]; then
    ...
    read -p "   Выберите чанкер[markdown/semantic] (По умолчанию: markdown): " CHUNKER
...
CMD="docker-compose exec rag-cli python main.py test $EVAL_MODE --chunker=$CHUNKER --retriever=$RETRIEVER --index-mode=$INDEX_MODE $FORCE_INDEX"
