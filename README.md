...
### Judge-метрики в отчёте (без сводного Score)

В **`output/report_*.md`** в сводке выводятся **отдельные** средние **Avg Faithfulness**, **Avg Relevance** и при **`evaluation_metrics.metrics.similarity: true`** — **Avg Similarity (LLM ↔ эталон)**. Отдельного агрегированного «Score» по шагам **нет** — сравнивайте конфигурации по тем осям, которые для вас важны (F, R, S, Hit, latency).
...
**Как интерпретировать:** для UX **меньше обычно лучше**. Если сравниваете два конфига: при сопоставимых **Faithfulness / Relevance / Similarity (если включена)** предпочтительнее **меньшая** задержка; рост качества по judge при росте latency часто нормален (судья, rerank, HyDE, тяжелее модель и т.д.).

### Как согласовывать метрики между собой

| Ситуация | Как читать |
|----------|------------|
| Hit Rate ↑, Faithfulness/Relevance ↓ | В выдаче чаще «цепляется» формулировка эталона, но итоговый ответ судья оценивает ниже (слабая релевантность вопросу или слабая опора на контекст). |
| Hit Rate ↓, Faithfulness/Relevance ↑ | Мало пересечения слов с эталоном в чанках, но ответ судье нравится (другая формулировка той же мысли или эвристика hit промахнулась). |
...
В **`evaluation_model`** задаётся лёгкая модель эмбеддингов для **вспомогательных** целей в проекте (не путать с **`embedding_model`** для индексации). **Метрики LLM-judge** в отчёте `test` настраиваются в **`evaluation_metrics`**, а не через эту модель. Актуальный id см. в `config.yaml`.
